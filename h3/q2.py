...
frs = frInput.read().decode('utf-8').splitlines()
enInput.close()
frInput.close()
...
enInput = open('dev.en','r')
frInput = open('dev.es','r')
Output = open('alignment_test.p2.out','w')
worke = enInput.read().splitlines()
works = frInput.read().decode('utf-8').splitlines()
for i in range(len(works)):
    ...
    for j in range(len(s2)):
        ...
        for k in range(len(s1)):
            ...
            if (t[s2[j]][s1[k]]*q[k][j][len(s1)][len(s2)]>maxs):
                maxs=t[s2[j]][s1[k]]*q[k][j][len(s1)][len(s2)]
                point=k
